...
/*
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'RACIALPOWERS')
BEGIN
	CREATE TABLE RACIALPOWERS
	(
		ID int IDENTITY(1,1) PRIMARY KEY,
		NAME nvarchar(100) NOT NULL,
		TYPE int NOT NULL,
...
IF NOT EXISTS (SELECT 1 FROM RACIALPOWERS)
BEGIN
	INSERT INTO RACIALPOWERS (NAME, TYPE, ACTIONTYPE, EFFECTTYPE, RANGE, AURARANGE, DESCRIPTION)
	VALUES 
	('Charming', 1, 0, 0, 0, 0, 'You gain proficiency in diplomacy.'),
	('Child of Darkness', 1, 0, 0, 0, 0, 'You gain darkvision.'),
	('Child of Light', 1, 0, 0, 0, 0, 'You may equip one additional blessing.'),
	('Dark Omen', 2, 3, 6, 0, 0, 'Cause the entire combat zone to be enshrouded in pitch blackness until the end of your next turn.'),
	('Deathblow', 2, 3, 2, 0, 0, 'Instantly kill a non-elite creature within range.'),
	('Enduring', 1, 0, 0, 0, 0, 'You gain proficiency in endurance.'),
	('Fast Hands', 2, 3, 2, 0, 0, 'You take an item within range that is within your reach and not equipped on a creature as if you made a successful thievery check against the item. You are not noticed when you use Fast Hands.'),
	('Flexible', 1, 0, 0, 0, 0, 'You gain +2 to every skill you are not proficient in.'),
	('Haunting', 2, 1, 1, 0, 0, 'While you are unconscious due to falling to 0 hit points or fewer during a Combat Encounter, you may continue combat like normal as a phantom spirit. Only the creature that dropped you to 0 hit points may see you, and you may only damage and interact with the same offending creature. 
You may not move beyond 20 squares of your physical body and the effect ends when you are revived or die.'),
	('Herbalism', 2, 4, 1, 0, 0, 'Regain 3 hit points.'),
	('Heroism', 2, 5, 1, 0, 0, 'Gain a Standard Action.'),
	('Intimidating', 1, 0, 0, 0, 0, 'You gain proficiency in intimidation.'),
	('Mighty', 1, 0, 0, 0, 0, 'You gain proficiency in athletics.'),
	('Overpower', 2, 1, 1, 0, 0, 'When you roll damage dice and dislike the result, you may reroll the damage dice and use either result. 
You may use Overpower twice per encounter.'),
	('Perfection', 2, 1, 1, 0, 0, 'When you roll to hit an elite creature and miss, you may cause yourself to automatically hit the creature instead of miss.'),
	('Regenerative', 1, 0, 0, 0, 0, 'You regain 1 hit point at the end of each of your turns during a combat encounter.'),
	('Stubbornness', 2, 1, 1, 0, 0, 'When you are afflicted with a negative effect (Slow, Immobilized, Mind Control, etc), immediately cause that effect to end. You may use this ability any time, even during an enemy''s turn.'),
	('Sturdy', 1, 0, 0, 0, 0, 'You gain +1 stamina and +2 hit points.'),
	('Superior', 1, 0, 0, 0, 0, 'You gain one additional specialization point.'),
	('Thieving', 1, 0, 0, 0, 0, 'You gain proficiency in thievery.'),
	('Versatile', 1, 0, 0, 0, 0, 'You are considered to have the Multiclass ability on your loadout. At Paragon level, you may choose Multiclass to gain access to a third class, or you may choose to gain Knighthood in either of the classes you gained from Versatile.')
END
GO


IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'RACES2RACIALPOWERS')
BEGIN
	CREATE TABLE RACES2RACIALPOWERS
	(
		RACEID_FK int NOT NULL FOREIGN KEY REFERENCES RACES(ID),
		RACIALPOWERID_FK int NOT NULL FOREIGN KEY REFERENCES RACIALPOWERS(ID)
	);
END
GO

IF NOT EXISTS (SELECT 1 FROM RACES2RACIALPOWERS)
BEGIN
	INSERT INTO RACES2RACIALPOWERS (RACEID_FK, RACIALPOWERID_FK)
	VALUES
	(1, 3),
	(1, 19),
	(1, 15),
...
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'CLASSPOWERS')
BEGIN
	CREATE TABLE CLASSPOWERS
	(
		ID int IDENTITY(1,1) PRIMARY KEY,
		CLASSID_FK int NOT NULL FOREIGN KEY REFERENCES CLASSES(ID),
		NAME nvarchar(100) NOT NULL,
		TYPE int NOT NULL,
		ACTIONTYPE int,
		EFFECTTYPE int,
...
IF NOT EXISTS (SELECT 1 FROM CLASSPOWERS)
BEGIN
	INSERT INTO CLASSPOWERS (CLASSID_FK, NAME, TYPE, ACTIONTYPE, EFFECTTYPE, RANGE, AURARANGE, DESCRIPTION, TIER, ACTIVE)
	VALUES
	(1, 'Chloroform', 2, 4, 2, 0, 0, 'Cause one creature within range to fall unconscious until the end of your next turn.', 1, 1),
	(1, 'Hand to Hand', 2, 3, 2, 0, 0, 'Disarm a creature within range, removing their ability to use their Basic Attack or any ability that requires a weapon to use. In addition, you strike their windpipe, removing their ability to make any sound. Both of these effects last until the end of your next turn.', 1, 1),
	(1, 'Escape Plan', 2, 4, 3, 0, 3, 'You and every willing ally within the aura may spend 1 Mana to successfully retreat to the place where you spent your last extended rest. If this place is more than 1 mile away, you instead travel two miles in that direction. 
This movement is unhindered and unchallenged and unfollowed, and takes the time it would take you to sprint the distance.', 1, 1),
	(1, 'Evasion', 1, 0, 0, 0, 0, 'Once per round, when you would be hit by an attack or ability, you may roll a dice. If the result is even, then you negate the incoming damage and effects. 
Prerequisite: You must be wearing Leather Armor to use Evasion.', 1, 1),
	(1, 'Investigative Expertise', 2, 4, 3, 0, 3, 'Within an area around you, dust for prints, pick up footprints, uncover hidden compartments, and gain truesight within the aura. If something can be found, you find it. 
To use Investigative Expertise, you require 10 minutes unmoved and uninterrupted. At the end of 5 minutes, Investigative Expertise is used.', 1, 1),
	(1, 'Leather Armor Specialization', 4, 0, 0, 0, 0, 'You gain proficiency with Leather Armor.', 1, 1),
	(0, '', 0, 0, 0, 0, 0, '', 1, 1)
END
GO
*/
...
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'RACES2POWERS')
BEGIN
	CREATE TABLE RACES2POWERS
	(
		RACEID_FK int NOT NULL FOREIGN KEY REFERENCES RACES(ID),
...
IF NOT EXISTS (SELECT 1 FROM RACES2POWERS)
BEGIN
	INSERT INTO RACES2POWERS (RACEID_FK, POWERID_FK)
	VALUES
	(1, 3),
	(1, 19),
	(1, 15),
...
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'CLASSES2POWERS')
BEGIN
	CREATE TABLE CLASSES2POWERS
	(
		CLASSID_FK int NOT NULL FOREIGN KEY REFERENCES CLASSES(ID),
		POWERID_FK int NOT NULL FOREIGN KEY REFERENCES POWERS(ID)
	);
...
IF NOT EXISTS (SELECT 1 FROM CLASSES2POWERS)
BEGIN
	INSERT INTO CLASSES2POWERS (CLASSID_FK, POWERID_FK)
	VALUES
	(1, 22),
	(1, 23),
	(1, 24),
