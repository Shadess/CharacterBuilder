...
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'RACEPOWERS')
BEGIN
	CREATE TABLE RACEPOWERS
	(
		ID int IDENTITY(1,1) PRIMARY KEY,
...
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'RACEPOWERATTRIBUTESMAP')
BEGIN
	CREATE TABLE RACEPOWERS
	(
		RACEPOWERSID_FK int NOT NULL FOREIGN KEY REFERENCES RACEPOWERS(ID),
		ATTRIBUTESID_FK int NOT NULL FOREIGN KEY REFERENCES ATTRIBUTES(ID)
	);
END
GO
...
/* POWER ATTRIBUTES */
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'ATTRIBUTES')
BEGIN
	...
	(
		...
		SPECIALIZATIONPOINT int NOT NULL DEFAULT 0,
		AWAKENINGPOINT int NOT NULL DEFAULT 0,
		ACROBATICS bit NOT NULL DEFAULT 0,
		ARCANA bit NOT NULL DEFAULT 0,
		ATHLETICS bit NOT NULL DEFAULT 0,
		BLUFF bit NOT NULL DEFAULT 0,
		DIPLOMACY bit NOT NULL DEFAULT 0,
		ENDURANCE bit NOT NULL DEFAULT 0,
		HEAL bit NOT NULL DEFAULT 0,
		HISTORY bit NOT NULL DEFAULT 0,
		INSIGHT bit NOT NULL DEFAULT 0,
		INTIMIDATE bit NOT NULL DEFAULT 0,
		...
		THIEVERY bit NOT NULL DEFAULT 0,
	);
END
GO
